...
let
  api = import (contrailPath + "/test/configuration/R3.2/api.nix") { inherit pkgs; };
  schemaTransformer = import (contrailPath + "/test/configuration/R3.2/schema-transformer.nix") { inherit pkgs; };
  ...
  machine = {pkgs, config, ...}: {
    imports = [ (contrailPath + "/modules/contrail-database-loader.nix") ];
    config = {
