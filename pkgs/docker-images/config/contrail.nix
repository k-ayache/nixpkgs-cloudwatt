  ...
  collector = pkgs.writeTextFile {
    ...
    text = config {
      ...
      conf = {
        ...
        REDIS = {
          #server ip would be 127.0.0.1 or 0.0.0.0 (otherwise won't work), and by default it is
          server = "127.0.0.1";
...
#          aaa_mode = "no-auth";
        ...
        REDIS = {
          #In code source server ip is fixed to localhost, it's useless to spesify it
